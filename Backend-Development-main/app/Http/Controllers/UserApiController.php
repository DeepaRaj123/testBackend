...
class UserApiController extends Controller
{
    ...
    public function send_request(Request $request) {
        ...
        $this->validate($request, [
                ...
                'use_wallet' => 'numeric',
                'payment_mode' => 'required|in:CASH,CARD,PAYPAL,Korttimaksu,Lasku,MobilePay,razorpay',
   ...
   //  dd($Providers);
        ...
        try{
            ...
            if(!empty($UserRequests)){
                ...
                foreach ($UserRequests as $key => $value) {
                    $UserRequests[$key]['address'] = Setting::get('address');
                        $UserRequests[$key]['tax_percentage'] = Setting::get('tax_percentage');
                }
            }
          
            return response()->json(['data' => $UserRequests , 'sos' => Setting::get('sos_number', '911'), 'cash' => (int)Setting::get('CASH', 1),'razorpay' => (int)Setting::get('razorpay', 0),'card' => (int)Setting::get('CARD', 0),'currency'=>Setting::get('currency','$'),'address'=>Setting::get('address','null'),'tax_percentage'=>Setting::get('tax_percentage',0)]);
    ...
    public function rate_provider(Request $request) {
		...
		(new SendPushNotification)->sendPushToProvider($UserRequest->provider_id,"Customer Successfully Rated");
    ...
    public function modifiy_request(Request $request) {
        ...
        $this->validate($request, [
                ...
                'payment_mode' => 'sometimes|nullable|in:CASH,CARD,PAYPAL,razorpay',
                'card_id' => ['required_if:payment_mode,CARD','exists:cards,card_id,user_id,'.Auth::user()->id],
            ]);
