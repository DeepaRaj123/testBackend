...
@section('scripts')
    ...
    <script type="text/jsx">
		...
		var SwitchState = React.createClass({
			...
			render: function(){
				...
				if(this.props.checkState != ""){
					...
					}else if((this.props.checkState.status == 'DROPPED' || this.props.checkState.status == 'COMPLETED') && this.props.checkState.payment_mode == 'Korttimaksu' && this.props.checkState.paid == 0){
						return (
							<div>
								<DroppedAndKorttimaksu checkState={this.props.checkState} currency={this.props.currency} />
							</div>
						);
					}else if((this.props.checkState.status == 'DROPPED' || this.props.checkState.status == 'COMPLETED') && this.props.checkState.payment_mode == 'Lasku' && this.props.checkState.paid == 0){
						return (
							<div>
								<DroppedAndLasku checkState={this.props.checkState} currency={this.props.currency} />
							</div>
						);
					}else if((this.props.checkState.status == 'DROPPED' || this.props.checkState.status == 'COMPLETED') && this.props.checkState.payment_mode == 'MobilePay' && this.props.checkState.paid == 0){
						return (
							<div>
								<DroppedAndMobilePay checkState={this.props.checkState} currency={this.props.currency} />
							</div>
						);
		...
		var DroppedAndKorttimaksu = React.createClass({
		...
		var DroppedAndLasku = React.createClass({
		...
		var DroppedAndMobilePay = React.createClass({
		...
		var DroppedAndRazorpay = React.createClass({
			...
			submitrazorpay: function(e){   
				let update_value = this.state.payable;
            
				call(update_value, this.props.checkState.user.email, this.props.checkState.user.first_name , this.props.checkState.id);		    
			},
			...
			render: function(){
				...
				<div>
					...
					<div className="status">
                    	...
                    	<button type="button" id="rzp-button1" onClick={this.submitrazorpay.bind(this)} data-bookingid={this.props.checkState.booking_id} className="full-primary-btn fare-btn">CONTINUE TO PAY - {this.props.currency}{this.state.payable.toFixed(fixval)}</button>  
                    </form>
		        </div>
				);
			}
